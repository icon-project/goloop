...
function join(){
    ...
    for i in $(seq 0 $((${GOLOOP_DOCKER_REPLICAS}-1)));do
        docker exec ${GOLOOP_DOCKER_PREFIX}-${i} goloop chain join \
            --genesis ${GOLOOP_GENESIS_STORAGE} \
            --seed "${GOLOOP_DOCKER_PREFIX}-0":8080 \
            --default_wait_timeout ${GOLOOP_DEF_WAIT_TIMEOUT} \
            --platform icon \
            --channel test
    done
}
