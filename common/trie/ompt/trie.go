...
type (
	mpt struct {
		...
		appliedPool map[string]trie.Object
		mutex       sync.Mutex
		store       db.Store
		db          db.DB
...
/*
 */
func newMpt(s db.Store, t reflect.Type, initialHash hash) *mpt {
	db, err := s.GetDB("")
	if err != nil {
		log.Fatalln("FAIL to get DB from ", s)
		return nil
	}
	return &mpt{
		committedHash: initialHash,
		requestPool:   make(map[string]trie.Object),
		appliedPool:   make(map[string]trie.Object),
		store:         s,
		db:            db,
		objType:       t,
	}
...
func (m *mpt) ensureObject(o trie.Object) trie.Object {
	if reflect.TypeOf(o) != m.objType {
		nv := reflect.New(m.objType.Elem())
		if no, ok := nv.Interface().(trie.Object); ok {
			if err := no.Reset(m.store, o.Bytes()); err == nil {
				return no
			}
		}
	}
...
func (m *mpt) get(n node, k []byte) (node, trie.Object, error) {
	...
	case *branch:
		if len(k) != 0 {
			...
			n.value = m.ensureObject(n.value)
			result = n.value
		}
	case *extension:
...
*/
func (m *mpt) Set(k []byte, v trie.Object) error {
...
func (m *mpt) GetSnapshot() trie.SnapshotForObject {
	mpt := newMpt(m.store, m.objType, m.committedHash)
	m.mutex.Lock()
	for k, v := range m.requestPool {
		mpt.requestPool[k] = v
	}
...
// TODO: proper error
func (m *mpt) Reset(immutable trie.ImmutableForObject) error {
	in, ok := immutable.(*mpt)
	if ok == false {
		return nil
	}
